            ...
            let Some(if_let_or_match) = IfLetOrMatch::parse(cx, init)
        {
            match if_let_or_match {
                ...
                IfLetOrMatch::Match(match_expr, arms, source) => {
                    ...
                    if pat_and_expr_can_be_question_mark(cx, pat_arm.pat, pat_arm.body).is_some() {
                        // We already lint the question_mark lint here
                        return;
                    }
...
pub fn pat_and_expr_can_be_question_mark<'a, 'hir>(
    ...
    if let PatKind::TupleStruct(pat_path, &[ref inner_pat], _) = pat.kind &&
